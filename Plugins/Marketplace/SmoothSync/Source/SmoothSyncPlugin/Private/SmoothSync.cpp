...
void USmoothSync::addState(SmoothState* state)
{
	...
	if (stateCount > 0)
	{
		// If the buffer is full, remove the last state
		...
		{
			if (stateBuffer[insertPos] == NULL || state->ownerTimestamp >= stateBuffer[insertPos]->ownerTimestamp)
			{
				break;
			}
		}
		// Shift all states after the insertPos to make room
		for (int i = std::min(stateCount, stateBufferLength - 1); i > insertPos; i--)
		{
