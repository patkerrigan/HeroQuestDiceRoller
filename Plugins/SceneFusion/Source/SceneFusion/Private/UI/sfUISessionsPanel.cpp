        ...
        [
            ...
            .OnGenerateWidget(SComboBox<TSharedPtr<sfProjectInfo>>::FOnGenerateWidget::CreateLambda(
                [this](TSharedPtr<sfProjectInfo> valuePtr)->TSharedRef<SWidget> {
                    FString label = valuePtr->CompanyName + " / " + valuePtr->Name;
                    return SNew(SBox)
                    .Padding(FMargin(2, 2))
                    .VAlign(VAlign_Center)
                    [
...
void sfUISessionsPanel::LANArea()
{
    ...
    [
        ...
        .Visibility_Lambda([this]() -> EVisibility {
            return (m_projectMap.Contains(m_projectId) && m_projectMap[m_projectId]->LANEnabled) ?
...
{
    m_newVersionPtr->SetMessage("Get Scene Fusion Version " + SceneFusion::Get().WebService->LatestVersion(),
        sfUIMessageBox::Icon::INFO, sfUIMessageBox::OnClickDelegate::CreateLambda([this]()
    {
        FString error = "";
        FPlatformProcess::LaunchURL(
            *FString("https://www.kinematicsoup.com/scene-fusion/download"), nullptr, &error);
    }));
}
...
void sfUISessionsPanel::SelectProject(TSharedPtr<sfProjectInfo> valuePtr, ESelectInfo::Type selectType)
{
    ...
    m_projectId = valuePtr->Id;
    m_objectLimit = valuePtr->ObjectLimit;
    sfConfig::Get().ProjectId = m_projectId;
    FString label = valuePtr->CompanyName + " / " + valuePtr->Name;
    m_selectedProjectPtr->SetText(FText::FromString(label));
}
...
FReply sfUISessionsPanel::StartSession()
{
    if (m_objectLimit >= 0)
    {
        ...
        if (numActors > m_objectLimit)
        {
            m_limitMessagePtr->SetMessage("Your level has " + FString::FromInt(numActors) + 
                " actors which exceeds the " + FString::FromInt(m_objectLimit) +
...
void sfUISessionsPanel::StopSessionReply(TSharedPtr<sfSessionInfo> sessionInfoPtr, const FString& errorMessage)
{
    ...
    else
    {
        ...
        if (m_projectMap.Contains(m_projectId))
        {
            projectInfoPtr = m_projectMap[m_projectId];
        }
...
void sfUISessionsPanel::GetSessionsReply(ProjectMap& projectMap, const FString& error)
{
    ...
    if (changed)
    {
        ...
        m_projectMap.KeySort([](const int& A, const int& B) {
            return A > B;
        });

        // Clear the selected project
        m_projectId = sfConfig::Get().ProjectId = m_projectId;
        if (m_projectId != -1 && !m_projectMap.Contains(m_projectId)) {
            m_projectId = -1;
        }
        ...
        if (m_projectMap.Num() == 0) {
            DisplayMessage(
                "You are not an editor in any projects. "
                "You must be invited to a project before you can use Scene Fusion.",
                sfUIMessageBox::WARNING);
            return;
        }
        m_projectComboPtr->SetSelectedItem(m_projectMap[m_projectId]);
...
{
    m_sessions.Empty();
    if (m_projectId != -1 && m_projectMap.Num() > 0  && m_projectMap.Contains(m_projectId)) {
        m_sessions.Append(m_projectMap[m_projectId]->Sessions);
    }
...
FReply sfUISessionsPanel::StartLANSession()
{
    ...
    sessionInfoPtr->ProjectId = m_projectId;
    if (m_projectMap.Contains(m_projectId))
    {
        sessionInfoPtr->ProjectName = m_projectMap[m_projectId]->Name;
    }
    ...
    jsonPtr->SetNumberField("projectId", m_projectId);
